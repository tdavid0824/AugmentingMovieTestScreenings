#!/usr/bin/env python3
"""
SIMULATION OF 40 SYNTHETIC MOVIES FOR ML TRAINING

Approach: Two-level hierarchical class-conditional sampling using
gaussian copulas. The copula approach preserves BOTH:
...
Outputs 3 file
  - scene_movie_metadata_v6_synthetic.csv   (40 rows)
  - participant_features_v6_synthetic.csv   (40 × 43 = 1720 rows)
  - movie_features_v6_synthetic.csv         (40 rows)
...
N_SYNTHETIC = 40
N_PARTICIPANTS = 43
SYNTHETIC_PARTICIPANT_ID_START = 1000
TIER_THRESHOLD = 7.5
CPI_BASE = 322.0
...
meta_cols, meta_rows = load_csv(DATA_DIR / "scene_movie_metadata_v6.csv")
print(f"  Metadata: {len(meta_rows)} movies × {len(meta_cols)} cols")

ppt_cols, ppt_rows = load_csv(DATA_DIR / "participant_features_v6.csv")
...
#PREPARE PARTICIPANT-LEVEL FEATURES PER TIER
...
#METADATA SAMPLING DICTIONARIES
...
out_meta = DATA_DIR / "scene_movie_metadata_v6_synthetic.csv"
...
out_ppt = DATA_DIR / "participant_features_v6_synthetic.csv"
...
mov_cols, _ = load_csv(DATA_DIR / "movie_features_v6.csv")
...
out_mov = DATA_DIR / "movie_features_v6_synthetic.csv"
